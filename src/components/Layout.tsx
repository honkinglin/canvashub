      ...
      <header className="sticky top-0 z-50 w-full border-b bg-[var(--topbar-bg)] border-[color:var(--surface-border)] backdrop-blur-xl">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between gap-2">
          ...
          <nav className="flex items-center gap-2 md:gap-3">
            <Link to="/" className="hidden sm:block text-sm font-semibold text-[var(--text-muted)] hover:text-[#0864ef] transition-colors px-2">
              {text.navGallery}
            </Link>
            ...
            <div className="ui-pill rounded-full p-1 flex items-center gap-1">
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-xs font-semibold transition-colors ${language === 'zh' ? 'ui-pill-active' : ''}`}
              >
                {text.langZh}
              </button>
              <button
                ...
                aria-label={text.navLanguageToggle}
                className={`px-2.5 py-1 rounded-full text-xs font-semibold transition-colors ${language === 'en' ? 'ui-pill-active' : ''}`}
              >
                {text.langEn}
              </button>
            </div>
            ...
            <button
              ...
              className="ui-pill rounded-full px-3 py-1.5 text-xs font-semibold transition-colors hover:text-[var(--text-main)] flex items-center gap-1.5"
            >
