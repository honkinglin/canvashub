...
const defaultConfig: CursorTrailConfig = {
...
const render: CanvasRenderFunction<CursorTrailConfig> = (canvas, ctx, initialConfig) => {
  ...
  return {
    ...
    updateConfig: (newConfig) => {
      ...
      config = { ...newConfig };
      if (Math.round(oldCount) !== Math.round(config.particleCount)) {
        createParticles();
      }
    },
  };
...
const generateCode = (config: CursorTrailConfig) => `
...
const CursorTrailBackground = () => {
  ...
  useEffect(() => {
    // Copy full logic from CanvasHub "Cursor Trail" for production use.
    console.log('Cursor Trail config', config);
  }, []);
