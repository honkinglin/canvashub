...
export const render: CanvasRenderFunction<PixelFieldConfig> = (canvas, ctx, initialConfig) => {
  ...
  const buildGrid = () => {
    ...
    if (pixelCtx) {
      imageData = pixelCtx.createImageData(Math.max(1, simW), Math.max(1, simH));
    } else {
      imageData = ctx.createImageData(Math.max(1, simW), Math.max(1, simH));
    }
  ...
  const rebuild = () => {
    ...
    viewW = Math.max(1, Math.round(canvas.width / dpr));
    viewH = Math.max(1, Math.round(canvas.height / dpr));
  ...
  const animate = (time: number) => {
    ...
    const nextSignature = `${w}x${h}:${config.rows}x${config.cols}:${config.spacing}:${config.margin}`;
  ...
  return {
    ...
    updateConfig: (newConfig) => {
      ...
      if (prev.particleColor !== config.particleColor) {
        color = hexToRgb(config.particleColor, { r: 220, g: 220, b: 220 });
      }
    },
  };
};
